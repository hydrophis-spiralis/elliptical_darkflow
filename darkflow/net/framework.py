...
class YOLO_orient(framework):
    constructor = yolo.constructor
    parse = ellipse_yolo.data.parse
    shuffle = yolov2.data.shuffle
    preprocess = yolo.predict.preprocess
    loss = ellipse_yolo.train.loss
    is_inp = yolo.misc.is_inp
    postprocess = ellipse_yolo.predict.postprocess
    _batch = ellipse_yolo.data._batch
    resize_input = yolo.predict.resize_input
    findboxes = ellipse_yolo.predict.findboxes
    process_box = yolo.predict.process_box
